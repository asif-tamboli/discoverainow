(() => {
 ...
 function data(){return [...rows.children].map(r=>({name:r.querySelector('.cr-name').value.trim()||'Unnamed tool',cat:r.querySelector('.cr-cat').value,price:Number(r.querySelector('.cr-price').value)||0,seats:Math.max(1,Number(r.querySelector('.cr-seats').value)||1)}));}
 function calc(){
   const d=data(),base=d.reduce((s,x)=>s+x.price*x.seats,0),tax=Number(document.getElementById('taxPct').value)||0,disc=Number(document.getElementById('discountPct').value)||0,monthly=base*(1+tax/100),annual=monthly*12,discounted=annual*(1-disc/100);
